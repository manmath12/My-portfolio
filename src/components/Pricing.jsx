        ...
        {pricingOptions.map((option, index) => (
          <div key={index} className="w-full sm:w-1/2 lg:w-1/3 p-2">
            <div className="p-10 border border-neutral-700 rounded-xl">
              ...
              <div className="mb-6">
              <img className="" src={option.image} alt={`${option.title} image`} className="inline-block mr-2 h-40 w-80" />
              </div>
                {option.title}
                {option.title === "Pro" && (
                  <span className="bg-gradient-to-r from-purple-500 to-purple-80 text-transparent bg-clip-text text-xl mb-4 ml-2">
                    (Most Popular)
                  </span>
                )}
              </p>
              <p className="mb-8">
                <span className="text-1xl mt-6 mr-2">{option.price}</span>
                 ...
                 <div className="mt-5">
                <span className="text-neutral-400 tracking-tight">Technology used :</span>
                </div>
              </p>
              <ul>
